...
const signToken = (id, email, firstName, lastName, role) => {
    const expiresIn = process.env.JWT_EXPIRES_IN?.trim() || "90d";

    return jwt.sign({ id, email, firstName, lastName, role }, process.env.JWT_SECRET, { expiresIn });
};

const createSendToken = (user, statusCode, res) => {
    const token = signToken(user._id, user.email, user.firstName, user.lastName, user.role);
    const cookieOptions = {
